...
const ProgramUpload: React.FC<ProgramUploadProps> = ({ onImportCircuits, onImportProgram, onCancel }) => {
  ...
  return (
            <div class="flex flex-col h-full bg-zinc-50">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 pb-32">
        ...
        <div className="bg-zinc-50 border border-zinc-100 rounded-2xl p-4 space-y-2">
          <p className="text-[10px] font-semibold text-sky-500 uppercase tracking-wide">Supported Formats</p>
          ...
          <p className="text-[10px] text-sky-400 font-medium">type must be: weight · reps · duration</p>
          <p className="text-[10px] text-sky-400 font-medium mt-1">Optional: add <strong>suggested_weight</strong> and/or <strong>suggested_reps</strong> columns to pre-fill values in the workout</p>
        </div>

        {/* Drop zone */}
        <div
          className="border-2 border-dashed border-slate-300 rounded-2xl p-8 text-center bg-white cursor-pointer hover:border-zinc-500 hover:bg-zinc-50/30 transition-all"
              ...
              {preview.isMultiWeek && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none text-sm font-semibold text-slate-900"
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                {preview.isMultiWeek && (
                  <div className="bg-zinc-50 rounded-xl p-2">
                    <p className="text-xl font-black text-sky-500">{preview.totalWeeks}</p>
                    <p className="text-[9px] font-black text-sky-400 uppercase tracking-wider">Weeks</p>
                  </div>
                )}
                      ...
                      <div className="divide-y divide-slate-50">
                        {days.map(entry => (
                          <div key={entry.day} className="px-4 py-3">
                            <p className="text-xs font-black text-sky-500 uppercase tracking-wider mb-1">Day {entry.day}</p>
